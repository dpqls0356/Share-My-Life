...
import {
  ...
  Switcher,
  Error,
} from "../components/auth-components.ts";
...
export default function CreateAccount() {
  ...
  const onSumbit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const userDocRef = doc(db, "users", credentials.user.uid); // 'users' 컬렉션의 특정 문서 참조를 생성
      await setDoc(
        userDocRef,
        {
          introduce: "",
          backgroundImg: downloadURL,
          userId: credentials.user.uid, // 여기에 userId 필드를 설정합니다.
        },
        { merge: true }
      ); // 데이터를 병합하여 저장
      //리다이렉션
      ...
      if (e instanceof FirebaseError) {
        ...
        setError(
          errors[e.code as keyof typeof errors] ||
  ...
  const errors = {
    ...
    "auth/weak-password":
      "취약한 비밀번호입니다. 길이를 더 길게 하거나 소문자, 특수문자, 숫자를 함께 사용하여 설정하세요.",
